...
const sortOptions = ["Most Recent", "Highest Bid", "Lowest Bid", "Deadline"];
const bidStatusOptions = ["All", "Accepted", "Pending", "Rejected"];
export type BidFilterValues = {
  sort: string;
  location: string;
  niches: string[];
  platforms: string[];
...
export default function BidFilter({
  onApply,
}: {
  onApply: (filters: BidFilterValues) => void;
}) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <h4 className="text-sm font-medium text-white">Bid Status</h4>
            {bidStatus !== "All" && (
              <button
                onClick={() => setBidStatus("All")}
        ...
        {/* Add <hr /> after this for separation if you wish */}
