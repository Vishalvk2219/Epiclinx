...
const platforms = ["All", "TikTok", "Instagram", "YouTube", "Facebook", "Snapchat", "Twitch", "Kick"]
...
export default function JobFilter({
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 z-50 h-full w-[90%] max-w-sm bg-[#333] p-4 overflow-y-auto transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            {location && (
              <button onClick={() => setLocation("")} className="text-xs text-[#00e0ca] underline">
                Clear
              </button>
            )}
          </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            {selectedNiches.length > 0 && (
              <button onClick={() => setSelectedNiches([])} className="text-xs text-[#00e0ca] underline">
                Clear
              </button>
            )}
          </div>
              ...
              return (
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-full transition-colors ${
                    selected ? "bg-epiclinx-teal text-black" : "bg-epiclinx-semiteal text-black"
                  }`}
                >
                  {niche}
                </button>
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            {selectedPlatforms.length > 0 && !selectedPlatforms.includes("All") && (
              <button onClick={() => setSelectedPlatforms(["All"])} className="text-xs text-[#00e0ca] underline">
                Clear
              </button>
            )}
          </div>
          <div className="flex flex-col gap-2">
            {platforms.map((platform) => (
              <label key={platform} className="flex items-center gap-2 text-sm cursor-pointer text-white">
                <input
                  type="checkbox"
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            {selectedDeliverables.length > 0 && (
              <button onClick={() => setSelectedDeliverables([])} className="text-xs text-[#00e0ca] underline">
                Clear
              </button>
            )}
          </div>
        ...
        <div className="flex justify-between items-center mt-6">
          <button onClick={clearAll} className="text-sm underline text-gray-400">
            Clear all
          </button>
          <button onClick={handleApply} className="bg-[#00e0ca] text-black px-5 py-2 rounded-full text-sm">
            Apply
          </button>
        </div>
      </div>
