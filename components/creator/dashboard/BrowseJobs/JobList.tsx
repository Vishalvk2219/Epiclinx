...
import { Briefcase, ChevronLeft, ChevronRight, DollarSign, Gavel, Search, Users } from "lucide-react"
...
export function JobsList({ show = true }: { show?: boolean }) {
    ...
    const filteredJobs = jobs
        .filter((job) => {
            ...
            // Filter by niches
            if (filters.niches.length > 0 && !job.niche?.some((n) => filters.niches.includes(n))) {
                return false
            }
            ...
            if (
                ...
                !job.platforms.some((p) => p.toLowerCase() === filters.platform.toLowerCase())
            ) {
                return false
            }

            // Filter by deliverables
            if (filters.deliverables.length > 0 && !job.deliverables?.some((d) => filters.deliverables.includes(d))) {
                return false
            }
        ...
        .sort((a, b) => {
            if (filters.sort === "Most Recent") {
                ...
                return Number.parseInt(b.payment.replace(/[^0-9]/g, "")) - Number.parseInt(a.payment.replace(/[^0-9]/g, ""))
            } else if (filters.sort === "Lowest Paid") {
                return Number.parseInt(a.payment.replace(/[^0-9]/g, "")) - Number.parseInt(b.payment.replace(/[^0-9]/g, ""))
            }
            return 0
        })
                ...
                {activeSection === "jobs" ? (
                    <>
                        ...
                        <div className="flex items-center justify-between my-6">
                            <div className="text-white text-xs font-light ml-3">{filteredJobs.length} jobs found</div>
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            currentPage === pageNum ? "bg-epiclinx-teal text-[#2A2A2A]" : "text-xs",
                                        )}
                                    >
                                        {pageNum}
                                    </button>
